...
def train(generator : Sequential, discriminator : Sequential, settings : dict):
    ...
    for epoch in range(1, settings["epochs"]+1):
        if settings["save_step"] > 0 and epoch % settings["save_step"] == 0:
            utils.console.info(f'Saved generator for epoch {epoch}')
            generator.save(f'{utils.RUN_FOLDER}/{utils.RUN_CONFIG}/generator_epoch_{epoch}.h5')

            utils.generate_test_images(generator, epoch)
        ...
        for b in range(batches_per_epoch):
            ...
            utils.console.debug(f'[epoch {epoch}/{settings["epochs"]}][batch {b+1}/{batches_per_epoch}] d_loss_real: {d_loss_real:.4f}, d_loss_fake: {d_loss_fake:.4f}, gan_loss: {gan_loss:.4f}, d_acc_fake: {d_acc_fake * 100:.2f}%, d_acc_real: {d_acc_real * 100:.2f}%')

        utils.console.info(f'[epoch {epoch}] d_loss_real: {d_loss_real:.4f}, d_loss_fake: {d_loss_fake:.4f}, gan_loss: {gan_loss:.4f}, d_acc_fake: {d_acc_fake * 100:.2f}%, d_acc_real: {d_acc_real * 100:.2f}%')
